...
public abstract class AbstractInterceptor<T extends Annotation> implements Serializable {
    ...
    public Object aroundInvoke(InvocationContext ic) throws Exception {
        ...
        if (method.getDeclaringClass().isInterface()) {
            method = ClassUtil.getMethodIfAvailable(ic.getTarget().getClass(), method.getName(),
        ...
        Signature signature = new Signature().setMethodName(method.getName())
                ...
                .setDeclaringTypeName(method.getDeclaringClass().getName()).setParameters(ic.getParameters());
        try {
