...
public class ServiceLocator implements Serializable {
    ...
    public <T> T getInstance(Class<T> classForInstance) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(String.format("[%s] Could not find custom configuration. Loading by class instance only",
                    classForInstance.getName()));
        }
    ...
    /**
     * Used in case when there are several implementations of interface. Bean name should be provided at this point to
    ...
    private void loadServiceLocatorProperties() {
        try {
            ...
            LOGGER.warn(String.format(
                    "Property file was not found or is not correct[%s]. Custom configuration will not be used.",
                    SERVICE_LOCATOR_PROPERTIES), e);
        }
    }
