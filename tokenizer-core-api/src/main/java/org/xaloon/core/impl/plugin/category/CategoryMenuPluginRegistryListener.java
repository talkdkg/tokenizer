...
/**
 * Category node observer is used to get notifications when plugin is inserted into plugin registry and create tree node
 * from category string
 * parameter For example we have two category strings: "/admin/utils", "/admin/test" Then there will be a tree created
 * with such nodes: admin / \
 * utils test
 * 
...
public class CategoryMenuPluginRegistryListener extends AbstractTreePluginRegistryListener {
    ...
    private GenericTreeNode<MenuItem> createTreeNode(String category) {
        ...
        menuItem.getParameters().add(
                new DefaultKeyValue<String, String>(CategoryConstants.PAGE_NAMED_PARAMETER_PARENT_CATEGORY, stringKey));
