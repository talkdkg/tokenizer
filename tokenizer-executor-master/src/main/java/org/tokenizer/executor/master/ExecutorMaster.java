...
/**
 * This guy currently does almost nothing; prototype for future needs. It exists
 * here for most important future tasks such as start/stop/watch MapReduce and
 * etc.
 * 
 * Here we will place a Listener for tasks such as Delete Task; only one single
 * Master node should be responsible.
 * 
 * @author Fuad
 * 
 */
public class ExecutorMaster {
  ...
  public ExecutorMaster(ZooKeeperItf zk, WritableExecutorModel model,
      ...
      String zkConnectString, int zkSessionTimeout, ExecutorInfo executorInfo,
      String hostName) {
    this.zk = zk;
    this.model = model;
  ...
  public void start() throws LeaderElectionSetupException, IOException,
    ...
    leaderElection = new LeaderElection(zk, "Executor Master",
        "/org/tokenizer/executor/masters", new MyLeaderElectionCallback());
  ...
  private class MyLeaderElectionCallback implements LeaderElectionCallback {
    public void activateAsLeader() throws Exception {
      ...
      // Start these processes, but it is not until we have registered our model
      // listener
      // that these will receive work.
      eventWorker.start();
      // jobStatusWatcher.start();
      
      Collection<TaskDefinition> taskDefinitions = model
          .getTaskDefinitions(listener);
      
      // push out fake events
      for (TaskDefinition taskDefinition : taskDefinitions) {
        eventWorker.putEvent(new ExecutorModelEvent(
            ExecutorModelEventType.TASK_DEFINITION_UPDATED, taskDefinition
                .getName()));
      }
    ...
    public void deactivateAsLeader() throws Exception {
      ...
      // thread: if there
      // was something running there that is blocked until the ZK connection
      // comes back up
  ...
  private class EventWorker implements Runnable {
    ...
    public void putEvent(ExecutorModelEvent event) throws InterruptedException {
      if (stop) {
    ...
    public void run() {
      ...
      while (!stop && !Thread.interrupted()) {
        try {
          ...
          // Warn if the queue is getting large, but do not do this just after
          // we started, because
          // on initial startup a fake update event is added for every defined
          // index, which would lead
          // to this message always being printed on startup when more than 10
          // indexes are defined.
          ...
          if (event.getType() == ExecutorModelEventType.TASK_DEFINITION_ADDED
            ...
            if (taskDefinition != null) {
              if (taskDefinition.getGeneralState() == TaskGeneralState.DELETE_REQUESTED
                ...
                prepareDeleteTaskDefinition(taskDefinition.getName());
          ...
          LOG.error("Error processing executor model event in ExecutorMaster.",
              t);
        }
      }
    }
  ...
  private class MyListener implements ExecutorModelListener {
    public void process(ExecutorModelEvent event) {
      try {
        // Let the events be processed by another thread. Especially important
        // since
  ...
  private void prepareDeleteTaskDefinition(String name) {
    ...
    try {
      TaskDefinition taskDefinition = model.getMutableTaskDefinition(name);
      if (taskDefinition.getGeneralState() == TaskGeneralState.DELETE_REQUESTED
          || taskDefinition.getGeneralState() == TaskGeneralState.DELETING) {
