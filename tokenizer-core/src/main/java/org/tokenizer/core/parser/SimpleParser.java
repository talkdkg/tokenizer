...
public class SimpleParser extends BaseParser {
   ...
   *          and attributes it processes. Instead, use
  ...
  public ParsedDatum parse(FetchedResult fetchedResult) throws Exception {
    ...
    InputStream is = new ByteArrayInputStream(fetchedResult.getContent(), 0,
        fetchedResult.getContent().length);
    
    try {
      URL baseUrl = getContentLocation(fetchedResult);
      metadata.add(Metadata.CONTENT_LOCATION, baseUrl.toExternalForm());
      
      Callable<ParsedDatum> c = new TikaCallable(_parser, _contentExtractor,
          _linkExtractor, is, metadata, isExtractLanguage());
