...
import org.tokenizer.core.parser.BaseLinkExtractor;
...
public class ParserPolicy implements Serializable {
    ...
    private int _maxParseDuration; // Max # of milliseconds to wait for parse to
                                   // complete a document.
    ...
    public ParserPolicy(int maxParseDuration) {
        this(maxParseDuration, BaseLinkExtractor.DEFAULT_LINK_TAGS, BaseLinkExtractor.DEFAULT_LINK_ATTRIBUTE_TYPES);
    }

    public ParserPolicy(int maxParseDuration, Set<String> linkTags, Set<String> linkAttributeTypes) {
        ...
        // Catch common error of specifying maxParseDuration in seconds versus
        // milliseconds
        if (maxParseDuration < 100) {
            throw new InvalidParameterException("maxParseDuration must be milliseconds, not seconds: "
                    + maxParseDuration);
        }
