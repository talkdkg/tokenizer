...
public class DocumentHelper {
    ...
    public static Element getElementChild(Element element, String name,
        ...
        if (required)
            throw new LocatedException("Missing element " + name + " in "
                    + element.getLocalName(),
                    LocationAttributes.getLocation(element));
        else
            return null;
    }
    ...
    public static Element[] getElementChildren(Element element, String name) {
        ...
        for (int i = 0; i < elements.length; i++) {
            if (elements[i].getLocalName().equals(name)
                    && elements[i].getNamespaceURI() == null) {
    ...
    public static String getAttribute(Element element, String name,
        ...
        if (!element.hasAttribute(name)) {
            if (required)
                throw new LocatedException("Missing attribute " + name
                        + " on element " + element.getLocalName(),
                        LocationAttributes.getLocation(element));
            else
                return null;
        }
    ...
    public static int getIntegerAttribute(Element element, String name)
        ...
        if (!element.hasAttribute(name))
            ...
            try {
                ...
                throw new LocatedException("Invalid integer value " + value
                        + " in attribute " + name,
                        LocationAttributes.getLocation(element));
            }
        }
    ...
    public static boolean getBooleanElement(Element element, String name,
            boolean defaultValue) throws Exception {
    ...
    public static String getStringElement(Element element, String name,
    ...
    public static int getIntegerElement(Element element, String name,
            int defaultValue) throws Exception {
        Element child = getElementChild(element, name, false);
        if (child == null)
            return defaultValue;
        ...
        try {
            ...
            throw new LocatedRuntimeException("Invalid integer value " + text
                    + " in element " + name,
                    LocationAttributes.getLocation(element));
        }
    }

    public static String getElementText(Element element, boolean required)
            throws Exception {
        ...
        if (required && text.length() == 0)
            throw new LocatedException("Missing text content in element "
                    + element.getLocalName(),
                    LocationAttributes.getLocation(element));
    ...
    public static Document parseDomWithLocationAttributes(File file)
    ...
    public static Document parseDomWithLocationAttributes(
            InputSource inputSource) throws SAXException,
            ParserConfigurationException, TransformerConfigurationException,
            IOException {
        XMLReader xmlReader = LocalSAXParserFactory.newXmlReader();
        TransformerHandler serializer = LocalTransformerFactory.get()
                .newTransformerHandler();
        DOMResult result = new DOMResult();
        serializer.setResult(result);
        LocationAttributes.Pipe locationPipe = new LocationAttributes.Pipe(
                serializer);
