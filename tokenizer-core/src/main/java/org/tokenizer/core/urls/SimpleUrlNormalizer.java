...
import org.tokenizer.core.util.StringUtils;
...
public class SimpleUrlNormalizer extends BaseUrlNormalizer {
    ...
    private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(SimpleUrlNormalizer.class);
    ...
    private static final Pattern DEFAULT_PAGE_PATTERN = Pattern
            .compile("/((?i)index|default)\\.((?i)js[pf]{1}?[afx]?|cgi|cfm|asp[x]?|[psx]?htm[l]?|php[3456]?)(\\?|&|#|$)");
    ...
    private static final Pattern SESSION_ID_PATTERN = Pattern
            .compile("(\\?|&)(?:(?i)sid|phpsessid|sessionid|session_id|bv_sessionid|jsessionid|-session|session|session_key)=.*?(&|#|$)");

    // Remove other common unwanted parameters from the query portion of a URL.
    private static final Pattern OTHER_IGNORED_QUERY_PARAMETERS_PATTERN = Pattern
            .compile("(\\?|&)(?:(?i)width|format|country|height|src|user|username|uname|return_url|returnurl|sort|sort_by|sortby|sort_direction|sort_key|order_by|orderby|sortorder|collate)=.*?(&|#|$)");
    ...
    private String encodeCodePoint(int codepoint) {
        try {
            ...
            LOG.error("Unexpected exception during URL encoding: " + e);
            return "";
        }
    ...
    public String decodeUrl(String url) {
        ...
        try {
            ...
            LOG.error("Unexpected exception during URL decoding: " + e);
            return url;
        }
    }
    ...
    public String normalize(String url) {
        ...
        try {
            ...
            LOG.debug("Malformed URL being returned without further processing: " + result);
            return result;
        }
        ...
        try {
            ...
            LOG.error("Unexpected exception during normalization: " + e);
            return result;
        }
