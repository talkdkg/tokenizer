...
public class V7Ini extends Ini {
    ...
    public static enum ZkParam {
        connectString,
        sessionTimeout
    }

    public static enum NlpParam {
        implementation
    }
    ...
    public String zkParam(ZkParam zkParam) {
        Section section = getSection("zookeeper");
        if (section == null) {
            log.warn("zookeeper section should not be null in V7ini");
            return null;
        }
        else {
            String value = section.get(zkParam.name());
            return value;
        }
    }

    public String nlpParam(NlpParam nlpParam) {
        Section section = getSection("nlp tools");
        if (section == null) {
            log.warn("nlp tools section should not be null in V7ini");
            return null;
        }
        else {
            String value = section.get(nlpParam.name());
            return value;
        }
    }
