...
public interface ZooKeeperItf extends Closeable {
     ...
     * 
     * <p>
     * Note that in case of connection loss, you are never sure if the operation succeeded or not, so it might be
     * executed twice. Therefore:
     * 
     * <ul>
     ...
     * <li>in case of creation of a sequential node, two nodes might have been created. If they are ephemeral, you can
     * use Stat.ephemeralOwner to find out the ones that belong to the current session. Otherwise, embed the necessary
     * identification into the name or data.
     * </ul>
     * 
     * <p>
     * Do not call this method from within a ZooKeeper watcher callback, as it might block for a longer time and hence
     * block the delivery of other events, including the Disconnected event.
     * 
     * <p>
     * <b>This is a Lily-specific method.</b>
     */
    ...
    void create(String path, byte[] data, List<ACL> acl, CreateMode createMode, AsyncCallback.StringCallback cb,
            Object ctx);
