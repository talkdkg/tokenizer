...
public class ZkLock {
    ...
    public static String lock(final ZooKeeperItf zk, final String lockPath)
        ...
        try {
            ...
            // 1. Call create( ) with a pathname of "_locknode_/lock-" and the
            ...
            while (true) {
                    ...
                    if (child.getThreadId() == threadId) {
                        ...
                        if (stat != null
                            ...
                            if (myLockName != null) {
                                ...
                                // to us.
                                // This means that the lock creation above was
                                // executed twice, which can occur
                                // in case of connection loss. Delete this node
                                // to avoid that otherwise it would
                                // never be released.
                ...
                if (stat == null) {
                    ...
                    // This means (I think) that the lock was removed
                    // (explicitly or through session expiration) between
                    // the moment we queried for the children and the moment we
                    // called exists()
                    // If the node does not exist, the watcher will still be
     ...
     * 
     * @param lockId
     *            the string returned by {@link ZkLock#lock}.
     * @param ignoreMissing
    ...
    private static class MyWatcher implements Watcher {
        ...
        public MyWatcher(final String path, final Object condition) {
            this.path = path;
    ...
    private static List<ZkLockNode> parseChildren(final List<String> children) {
    ...
    private static class ZkLockNode implements Comparable<ZkLockNode> {
        ...
        public ZkLockNode(final String name) {
            this.name = name;
        ...
        @Override
        public int compareTo(final ZkLockNode o) {
