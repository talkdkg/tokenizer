...
/**
 * Simple leader election system, not optimized for large numbers of potential
...
public class LeaderElection {
    ...
    private final ChildrenWatcher watcher = new ChildrenWatcher();
    private final ConnectStateWatcher connectStateWatcher = new ConnectStateWatcher();
    private boolean stopped = false;
    private final LeaderProvisioner leaderProvisioner = new LeaderProvisioner();
     ...
     * 
     * @param position
     *            a name for what position this leader election is about, used
     *            in informational messages.
     * @param electionPath
     *            path under which the ephemeral leader election nodes should be
    ...
    public void stop() throws InterruptedException {
        // Note that ZooKeeper does not have a way to remove watches (see
        // ZOOKEEPER-422)
        stopped = true;
    ...
    private void proposeAsLeader() throws LeaderElectionSetupException, InterruptedException, KeeperException {
        ...
        try {
            ...
            // (we do not know it). Therefore,
            // retrying upon connection loss is important, so that we can
            // continue with watching the leaders.
            // Later on, we do not look at the name of the node we created here,
            // but at the owner.
    ...
    private synchronized void watchLeaders() {
        try {
            // Here the code could be improved: providing the watcher here can
            // give the so-called
            // "herd-effect", especially when there are many potential leaders.
            List<String> children = zk.getChildren(electionPath, watcher);
            // The child sequence numbers are fixed-with, prefixed with zeros,
            ...
            // While we could compare the leader name with our own ephemeral
            // node name, it is safer to compare
            // with the Stat.ephemeralOwner field. This is because the creation
            // of the ephemeral node might have
            // failed with a ConnectionLoss exception, in which case we might
            // have retried and hence have two
            ...
            // If the exception happened on the zk.getChildren() call, then the
            // watcher will not have been
            // set, and we not be notified of future changes anymore. Thus we
            // will not know anymore if we could
            // be the leader, but if this is only a disconnected exception, we
            // still could be without knowing!
            // However, above we keep retrying the operation forever, so we will
            // only get here if the session
            // is really expired.
            LOG.error("Error getting children of path " + electionPath, e);
        }
    }
    ...
    public class ConnectStateWatcher implements Watcher {
        ...
        public void process(final WatchedEvent event) {
            ...
            if (event.getType() == EventType.None
                ...
                // get a connected event on this watcher once we are
                // reconnected.
    ...
    /**
     * Activates or deactivates the leader. This is done in a separate thread,
     * rather than in the ZooKeeper event thread, in order not to block delivery
     * of messages to other ZK watchers. A simple solution to this would be to
     * simply launch a thread in the LeaderElectionCallback methods. But then
     * the handling of the activateAsLeader() and deactivateAsLeader() could run
     * in parallel, which we do not desire. An improvement would be to put their
     * processing in a queue. But when we have a lot of disconnected/connected
     * events in a short time frame, faster than they are processed, it would
     * not make sense to process them one by one, we are only interested in
     * bringing the leader to latest requested state.
     * 
     * Therefore, the solution used here just keeps a 'requiredState' variable
     * and notifies a monitor when it is changed.
     */
    private class LeaderProvisioner implements Runnable {
        ...
        private volatile boolean stop; // do not rely only on Thread.interrupt
                                       // since some libraries eat interruptions
        ...
        public void setRequiredState(final LeaderState state) {
            synchronized (stateLock) {
