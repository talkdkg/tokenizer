 ...
 * 
 * <ul>
 * <li>on startup (= constructor) wait for the ZK connection to come up, if it
 * does not come up within the session timeout an exception will be thrown. This
 * avoids the remainder of the application starting up in the absence of a valid
 * ZK connection.
 * <li>when the session expires or the ZK connection is lost for longer than the
 * session timeout, it will shut down the application.
 * </ul>
 * 
 * <p>
 * So this is a good solution for applications which can not function in absence
 * of ZooKeeper.
 */
public class StateWatchingZooKeeper extends ZooKeeperImpl {

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory
            .getLogger(StateWatchingZooKeeper.class);
    ...
    public StateWatchingZooKeeper(final String connectString,
    ...
    public StateWatchingZooKeeper(final String connectString,
        ...
        if (zk.getState() != CONNECTED) {
            ...
            throw new IOException(
                    "Failed to connect with Zookeeper within timeout "
                            + startupTimeOut + ", connection string: "
                            + connectString);
        }
        LOG.info("ZooKeeper session ID is 0x"
        ...
        public void process(final WatchedEvent event) {
            ...
            try {
                ...
                if (event.getState() == Expired
                    ...
                    // StateWatcher thread).
                    sessionTimeout = negotiatedSessionTimeout > 0 ? negotiatedSessionTimeout
                            : requestedSessionTimeout;
                    if (negotiatedSessionTimeout == 0) {
                        // We could consider not even distributing this event
                        // further, but not sure about that, so
                        // just logging it for now.
                        LOG.info("The negotiated ZooKeeper session timeout is "
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (startNotConnected + margin < System.currentTimeMillis()) {
                    endProcess("ZooKeeper connection lost for longer than "
                            + margin
