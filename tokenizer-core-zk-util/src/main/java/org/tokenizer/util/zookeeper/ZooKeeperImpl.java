 ...
 * 
 * <p>
 * To wait until the ZK connection is established, use
 * {@link ZkUtil#connect(String, int)}.
 * 
 * <p>
...
public class ZooKeeperImpl implements ZooKeeperItf {
    ...
    public ZooKeeperImpl(final String connectString, final int sessionTimeout)
            throws IOException {
        this.delegate = new ZooKeeper(connectString, sessionTimeout,
                new MyWatcher());
    }
    ...
    public void waitForConnection() throws InterruptedException {
        ...
        if (stop)
            throw new InterruptedException(
                    "This ZooKeeper handle is shutting down.");
    }
    ...
    @Override
    public <T> T retryOperation(final ZooKeeperOperation<T> operation)
        ...
        while (true) {
            ...
            if (tryCount > 3) {
                LOG.warn("ZooKeeper operation attempt " + tryCount
    ...
    @Override
    public void delete(final String path, final int version,
            final AsyncCallback.VoidCallback cb, final Object ctx) {
    ...
    @Override
    public void exists(final String path, final Watcher watcher,
            final AsyncCallback.StatCallback cb, final Object ctx) {
        delegate.exists(path, watcher, cb, ctx);
    }

    @Override
    public void exists(final String path, final boolean watch,
            final AsyncCallback.StatCallback cb, final Object ctx) {
    ...
    @Override
    public void getData(final String path, final Watcher watcher,
            final AsyncCallback.DataCallback cb, final Object ctx) {
        delegate.getData(path, watcher, cb, ctx);
    }

    @Override
    public void getData(final String path, final boolean watch,
            final AsyncCallback.DataCallback cb, final Object ctx) {
    ...
    public void setData(final String path, final byte[] data,
            final int version, final AsyncCallback.StatCallback cb,
            final Object ctx) {
    ...
    @Override
    public void getACL(final String path, final Stat stat,
            final AsyncCallback.ACLCallback cb, final Object ctx) {
    ...
    public void getChildren(final String path, final Watcher watcher,
            final AsyncCallback.ChildrenCallback cb, final Object ctx) {
    ...
    @Override
    public void getChildren(final String path, final Watcher watcher,
            final AsyncCallback.Children2Callback cb, final Object ctx) {
        delegate.getChildren(path, watcher, cb, ctx);
    }
    ...
    @Override
    public void sync(final String path, final AsyncCallback.VoidCallback cb,
            final Object ctx) {
    ...
    public class MyWatcher implements Watcher {
        ...
        public void process(final WatchedEvent event) {
            ...
            if (event.getState() == Watcher.Event.KeeperState.Disconnected) {
                ...
                System.err
                        .println("ZooKeeper session expired at " + new Date());
                printConnectMsg = true;
