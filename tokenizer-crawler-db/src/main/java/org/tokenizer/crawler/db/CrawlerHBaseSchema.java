...
public class CrawlerHBaseSchema {
    ...
    static {
        ...
        webpageTableDescriptor.addFamily(new HColumnDescriptor(
                WebpageCf.DATA.bytes, 1, Compression.Algorithm.GZ.getName(),
    ...
    private static final HTableDescriptor xmlTableDescriptor;
    static {
        xmlTableDescriptor = new HTableDescriptor(Table.XML.bytes);
        xmlTableDescriptor.addFamily(new HColumnDescriptor(XmlCf.DATA.bytes, 1,
                Compression.Algorithm.NONE.getName(), false, true,
                HConstants.FOREVER, HColumnDescriptor.DEFAULT_BLOOMFILTER));
    }
    ...
    public static enum WebpageColumn {
        URL("url"), TIMESTAMP("timestamp"), CONTENT("content"), CHARSET(
                "charset");
        public final byte[] bytes;
        public final String name;
