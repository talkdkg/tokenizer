...
public class UrlRecordDecoder {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(UrlRecordDecoder.class);
  ...
  private static String decodeRecordIdInternal(byte[] rowKey)
    ...
    for (int i = 0; i < rowKey.length; i++) {
      if (rowKey[i] == DOT_BYTE) {
        ...
        suffix = new String(rowKey, i + 1, rowKey.length - i - 1, CHARSET);
  ...
  private static List<byte[]> getHostParts(String host)
    ...
    parts.add(part);
    if (LOG.isTraceEnabled()) LOG.trace("Uninverted: {}", arraysToString(parts));
    return parts;
  ...
  public static List<byte[]> getHostPartsInverted(String host)
    ...
    if (LOG.isTraceEnabled()) LOG.trace("Inverted: {}", arraysToString(partsInverted));
  ...
  private static final byte[] encodeInternal(String url)
    ...
    if (url == null || url.length() == 0 || !url.startsWith(URL_PREFIX)) return null;
    ...
    if (LOG.isTraceEnabled()) LOG.trace("Encoded URL: {}", arrayToString(encoded));
