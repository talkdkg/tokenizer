...
public class XmlRecord implements Serializable {
    ...
    public XmlRecord(final byte[] digest, final Date timestamp, final byte[] hostInverted_parseAttemptCounter,
            final byte[] content) {
        this.digest = digest;
        this.timestamp = timestamp;
        this.hostInverted = Arrays.copyOfRange(hostInverted_parseAttemptCounter, 0,
                hostInverted_parseAttemptCounter.length - 4);
        this.host = HttpUtils.getHostUninverted(hostInverted);
        byte[] parseAttemptCounterBytes = Arrays.copyOfRange(hostInverted_parseAttemptCounter,
                hostInverted_parseAttemptCounter.length - 4, hostInverted_parseAttemptCounter.length);
        this.parseAttemptCounter = HttpUtils.bytesToInt(parseAttemptCounterBytes);
        this.content = content;
    }
    ...
    public String toString() {
        try {
            return "XmlRecord [digest=" + Arrays.toString(digest) + ", host=" + host + ", hostInverted="
                    + Arrays.toString(hostInverted) + ", timestamp=" + timestamp + ", content="
                    + (new String(content, "UTF-8")) + ", parseAttemptCounter=" + parseAttemptCounter + "]";
