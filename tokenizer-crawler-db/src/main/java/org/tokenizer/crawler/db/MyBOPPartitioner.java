...
public class MyBOPPartitioner implements Partitioner {
    ...
    private static final ByteOrderedPartitioner partitioner = new ByteOrderedPartitioner();
    private static final MyBOPPartitioner instance = new MyBOPPartitioner();

    public static Partitioner get() {
        return instance;
    }

    public MyBOPPartitioner() {
    ...
    public List<TokenRange> splitTokenRange(String first, String last, int count) {
        ...
        List<String> splits = splitRange(new BigInteger(first), new BigInteger(
                last), count);
        Iterator<String> iter = splits.iterator();
        String current = iter.next();
        while (iter.hasNext()) {
            String next = iter.next();
            tokens.add(new TokenRangeImpl(current, next,
    ...
    public static List<String> splitRange(BigInteger first, BigInteger last,
            int count) {
        List<String> tokens = Lists.newArrayList();
        tokens.add(first.toString());
        BigInteger delta = (last.subtract(first).divide(BigInteger
                .valueOf((long) count)));
