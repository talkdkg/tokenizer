...
public class MyVaadinServlet extends org.vaadin.dontpush.server.DontPushOzoneServlet {
  ...
  public void init(javax.servlet.ServletConfig servletConfig)
    ...
    try {
      ...
      // Disable the servlet connectors: the Servlet container is our server.
  ...
  private File findLocalMavenRepository() {
    ...
    if (mavenSettingsFile.exists()) {
      try {
        ...
        // Usage of the POM namespace in settings.xml is optional, so also try
        // without namespace
        localRepository = xpath.evaluate("string(/settings/localRepository)",
            document);
        if (localRepository != null && localRepository.length() > 0) {
          return new File(localRepository);
        }
